...
async def test_create_menu(async_client):
...
async def test_read_menus(async_client):
...
async def test_read_menu(async_client):
    data_menu = {'title': 'My menu 1', 'description': 'My menu description 1'}
    response_menu = await async_client.post('/api/v1/menus', json=data_menu)
    assert response_menu.status_code == 201
...
async def test_update_menu(async_client):
    # Создаем новое меню
    data_menu = {'title': 'My menu 1', 'description': 'My menu description 1'}
    response_menu = await async_client.post('/api/v1/menus', json=data_menu)
    assert response_menu.status_code == 201
    ...
    response = await async_client.patch(f'/api/v1/menus/{menu_id}', json=data_update)
    assert response.status_code == 200
    menu = schemas.Menu(**response.json())
    assert menu.id == uuid.UUID(str(menu_id))  # преобразуем строку в UUID
...
async def test_delete_menu(async_client):
    # Создаем новое меню
    data = {'title': 'My menu 1', 'description': 'My menu description 1'}
    response_menu = await async_client.post('/api/v1/menus', json=data)
    assert response_menu.status_code == 201
    # Получаем ID созданного menu
    menu_id = response_menu.json()['id']
    # Удаляем menu
    response_menu = await async_client.delete(f'/api/v1/menus/{menu_id}')
    assert response_menu.status_code == 200
    # Получаем данные удаленного menu
    response_menu = await async_client.get(f'/api/v1/menus/{menu_id}')
